    ...
    public ActionForward executeNexusE2EAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response,
        ...
        if (searchFor != null && searchFor.equals("message") && command != null && command.equals("requeue")) {
            ...
            for (String messageIdent : values) {
                ...
                if (st.countTokens() != 4) {
                    ActionMessage errorMessage = new ActionMessage("generic.error", "Can't re-queue Message: >" + messageIdent + "<");
                ...
                if (messageContext != null) {
                    ...
                    String logMessage = "Message: " + messageId + " could not be found in database, cancelled re-queueing!";
        ...
        if (searchFor != null && searchFor.equals("message") && command != null && command.equals("stop")) {
            ...
            for (String messageIdent : values) {
                StringTokenizer st = new StringTokenizer(messageIdent, "|");
                if (st.countTokens() != 4) {
                    ActionMessage errorMessage = new ActionMessage("generic.error", "cant stop Message: >" + messageIdent + "<");
                    errors.add(ActionMessages.GLOBAL_MESSAGE, errorMessage);
                    addRedirect(request, URL, TIMEOUT);
                    continue;
                }
        ...
        if (searchFor != null && searchFor.equals("conversation") && command != null && command.equals("delete")) {
            ...
            for (String messageIdent : values) {
                ...
                StringTokenizer st = new StringTokenizer(messageIdent, "|");
                if (st.countTokens() != 3) {
                    ActionMessage errorMessage = new ActionMessage("generic.error", "Can't delete conversation: >" + messageIdent + "<");
                    errors.add(ActionMessages.GLOBAL_MESSAGE, errorMessage);
                    addRedirect(request, URL, TIMEOUT);
                    continue;
                }
