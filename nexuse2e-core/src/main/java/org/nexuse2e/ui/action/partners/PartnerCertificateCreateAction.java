...
public class PartnerCertificateCreateAction extends NexusE2EAction {
    ...
    public ActionForward executeNexusE2EAction(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            ActionMessage errorMessage = new ActionMessage("generic.error", "Unable to unpack Certificate File (" + fileName + ")!");
            errors.add(ActionMessages.GLOBAL_MESSAGE, errorMessage);
            addRedirect(request, URL, TIMEOUT);
            return error;
        }

        PartnerPojo partner = engineConfiguration.getPartnerByPartnerId(partnerId);

        CertificatePojo cPojo = new CertificatePojo();
        cPojo.setType(CertificateType.PARTNER.getOrdinal());
        cPojo.setName(alias);
        ...
        engineConfiguration.getCertificates(CertificateType.ALL.getOrdinal(), null).add(cPojo);
