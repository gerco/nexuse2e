...
public class FtpReceiverService extends AbstractService implements ReceiverAware {
    ...
    public void fillParameterMap(Map<String, ParameterDescriptor> parameterMap) {
        parameterMap.put(FTP_ROOT_PARAM_NAME,
                new ParameterDescriptor(ParameterType.STRING, "Root directory", "FTP upload root directory", new File("").getAbsolutePath()));
        ...
        parameterMap.put(CLIENT_AUTH_PARAM_NAME, new ParameterDescriptor(ParameterType.BOOLEAN, "Client authentication", "Require client authentication",
                Boolean.TRUE));
        parameterMap.put(IMPLICIT_PARAM_NAME, new ParameterDescriptor(ParameterType.BOOLEAN, "Implicit security", "Implicit security (default is on)",
                Boolean.TRUE));

        parameterMap.put(FTP_PORT_PARAM_NAME, new ParameterDescriptor(ParameterType.STRING, "FTP Port", "FTP port (default is 21)", "21"));
        parameterMap.put(FTPS_PORT_PARAM_NAME, new ParameterDescriptor(ParameterType.STRING, "FTPS Port", "FTPS port (default is 990)", "990"));
        final ParameterDescriptor certsParamDesc = new ParameterDescriptor(ParameterType.LIST, "Server certificate",
                "Use this certificate for server authentication", new ListParameter());
        ...
        if (getStatus().getValue() < BeanStatus.STARTED.getValue()) {
            try {
                ...
                if (!"ftp".equals(ftpTypeSel.getSelectedValue())) { // SSL
                    properties.setProperty("config.listeners.default.implicit-ssl", ((Boolean) getParameter(IMPLICIT_PARAM_NAME)).toString());

                    properties.setProperty("config.socket-factory.nxssl.port", (String) getParameter(FTPS_PORT_PARAM_NAME));
                    String certId = certSel.getSelectedValue();
                    properties.setProperty("config.socket-factory.nxssl.certificate-id", certId == null ? "" : certId);
                    properties.setProperty("config.socket-factory.nxssl.ssl-protocol", "TLS");
                    properties.setProperty("config.socket-factory.nxssl.client-authentication", ((Boolean) getParameter(CLIENT_AUTH_PARAM_NAME)).toString());

                    properties.setProperty("config.data-connection.ssl.ssl-protocol", "TLS");
                    properties.setProperty("config.data-connection.ssl.client-authentication", ((Boolean) getParameter(CLIENT_AUTH_PARAM_NAME)).toString());
                }
                ...
                properties.setProperty("config.ftplet.f0.errorDir", (String) getParameter(FTP_ERROR_DIR_PARAM_NAME));
                properties.setProperty("config.ftplet.f0.errorDir", (String) getParameter(FTP_ERROR_DIR_PARAM_NAME));
     ...
     *            The <code>TransportReceiver</code> to dispatch to.
     */
    private static void processFile(File file, File errorDir, String partnerId, TransportReceiver transportReceiver) {
    ...
    public static class TransactionHandler extends DefaultFtplet {
        ...
        public FtpletEnum onLogin(FtpRequest request, FtpResponse response) throws FtpException, IOException {
            ...
            if (!homeDir.exists()) {
                ...
                if (!succ) {
                    LOG.error("Could not create FTP home directory " + homeDir.getAbsolutePath() + " for user " + user.getName());
