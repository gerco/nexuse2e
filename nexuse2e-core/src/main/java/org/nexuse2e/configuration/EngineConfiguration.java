...
public class EngineConfiguration implements ConfigurationAccessService {
    ...
    public void createConfiguration() throws NexusException {
        ...
        BackendPipeline statusUpdatePipeline = null;
        ...
        while (choreographiesI.hasNext()) {
            ...
            if (actions != null) {
                ...
                while (actionsI.hasNext()) {
                    ...
                    statusUpdatePipeline = new BackendPipeline();
                    statusUpdatePipeline.setKey(actionSpecificKey);
                    ...
                    PipelinePojo statusUpdatePipelinePojo = action.getStatusUpdatePipeline();
                    ...
                    if ((statusUpdatePipelinePojo == null) || (statusUpdatePipelinePojo.getPipelets().size() == 0)) {
                        LOG.warn("No pipelets found for status pipeline for action: " + action.getName());
                    }
                    ...
                    // status update
                    pos = 0;
                    if (statusUpdatePipelinePojo != null) {
                        forwardPipelets = statusUpdatePipelinePojo.getForwardPipelets();
                        pipeletCount = forwardPipelets.size();
                        pipelets = new Pipelet[pipeletCount > 0 ? pipeletCount - 1 : 0];
                        ...
                        for (PipeletPojo pipeletPojo : forwardPipelets) {
                            ...
                            if (pos == 0) {
                                statusUpdatePipeline.setPipelineEndpoint(pipelet);
                            } else {
                                pipelets[pos - 1] = pipelet;
                            }
                            pos++;
        ...
        for (PipelinePojo pipelinePojo : frontendPipelines) {
            ...
            try {
                ...
                if (pipelinePojo.isOutbound()) {
                    ...
                    for (PipeletPojo pipeletPojo : forwardPipeletList) {
                        LOG.trace("Pipelet: " + pipeletPojo.getName() + " - " + pipeletPojo.getPosition() + " - " + pipeletPojo.getClass());
                        Pipelet pipelet = getPipeletInstanceFromPojo(pipeletPojo);
        ...
        staticBeanContainer.getFrontendOutboundDispatcher().setFrontendOutboundPipelines(frontendOutboundPipelinesArray);
            ...
            for (ServicePojo servicePojo : getServices()) {
                try {
                    ...
                    if (service instanceof AbstractControllerService) {
                        ...
                        Service wrappedService = Engine.getInstance().getEngineController().getServiceWrapper(service);
                    ...
                    if (service instanceof ApplicationObjectSupport) {
                        ((ApplicationObjectSupport) service).setApplicationContext(Engine.getInstance().getApplicationContext());
                    }
                } catch (NexusException e) {
    ...
    public Map<ActionSpecificKey, BackendPipeline> getStatusUpdatePipelines() {

        return statusUpdatePipelines;
    }

    public void setStatusUpdatePipelines(Map<ActionSpecificKey, BackendPipeline> statusUpdatePipelines) {

        this.statusUpdatePipelines = statusUpdatePipelines;
    }

    public Map<String, StatusUpdateSerializer> getStatusUpdateSerializers() {

        return statusUpdateSerializers;
    }
        ...
        for (Iterator<ComponentPojo> iter = components.iterator(); iter.hasNext();) {
            ...
            try {
                ...
                if (tempObject instanceof Pipelet) {
                    ...
                    LOG.trace("Pipelet " + componentPojo.getClassName() + " - is frontend: " + pipelet.isFrontendPipelet());
    ...
    public List<CertificatePojo> getCertificates(int type, Comparator<CertificatePojo> comparator) throws NexusException {
        ...
        if (type != CertificateType.ALL.getOrdinal()) {
    ...
    public void deletePartner(PartnerPojo partner) throws ReferencedPartnerException, NexusException {
        ...
        if (oldPartner != null) {
            ...
            getCertificates(CertificateType.ALL.getOrdinal(), null).removeAll(partner.getCertificates());
    ...
    public void updateCertificates(List<CertificatePojo> certs) throws NexusException {
        ...
        for (CertificatePojo certificate : certs) {
            ...
            getCertificates(CertificateType.ALL.getOrdinal(), null).add(certificate);
    ...
    public KeyStore getCacertsKeyStore() throws NexusException {
        ...
        try {
            ...
            List<CertificatePojo> certs = getCertificates(CertificateType.CA.getOrdinal(), null);
