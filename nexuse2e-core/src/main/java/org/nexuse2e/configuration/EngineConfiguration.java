...
public class EngineConfiguration implements ConfigurationAccessService {
            ...
            if ( actions != null ) {
                ...
                while ( actionsI.hasNext() ) {
                    ...
                    if ( statusUpdatePipelinePojo != null ) {
                        ...
                        staticBeanContainer.getManagableBeans().put(
                                statusUpdatePipelinePojo.getName() + "-" + statusUpdatePipeline.getKey() + "-"
                                        + Constants.POSTFIX_STATUS_UPDATE_PIPELINE, statusUpdatePipeline );
                    }
                }
        ...
        for ( PipelinePojo pipelinePojo : frontendPipelines ) {
            ...
            try {
                ...
                if ( pipelinePojo.isOutbound() ) {
                    ...
                    frontendPipeline.setReturnPipelineEndpoint( getStaticBeanContainer()
                            .getFrontendOutboundResponseEndpoint() );
                } else {
                        ...
                        if ( i >= 0 ) {
                            ...
                            TransportReceiver transportReceiver = (TransportReceiver) Engine.getInstance()
                                    .getEngineController().getTransportReceiver( urlAppendix,
                                            pipeletPojo.getComponent().getClassName() );
                    ...
                    frontendPipeline.setReturnPipelets( returnPipelets );
                    frontendPipeline.setReturnPipelineEndpoint( getStaticBeanContainer()
                            .getFrontendInboundResponseEndpoint() );
                }
    ...
    public void deleteChoreography( ChoreographyPojo choreography ) throws ReferencedChoreographyException,
        ...
        List<ConversationPojo> conversations = Engine.getInstance().getTransactionService()
                .getConversationsByChoreography( choreography );
        ...
        if ( !pipeline.isFrontend() ) {
            ...
            for ( ChoreographyPojo choreography : getChoreographies() ) {
                for ( ActionPojo action : choreography.getActions() ) {
                    if ( action != null
                            && ( ( action.getInboundPipeline() != null && action.getInboundPipeline().getNxPipelineId() == id ) || ( action
                                    .getOutboundPipeline() != null && action.getOutboundPipeline().getNxPipelineId() == id ) ) ) {
            ...
            if ( descriptor != null ) {
                ...
                for ( GenericParamPojo value : values ) {
                    if ( ( ( value.getTag() == null && tag == null ) || ( value.getTag() != null && value.getTag()
                        ...
                        resultMap.put( value.getParamName(), getParameterValue( descriptor.getParameterType(), value
                                .getValue() ) );
                        isUpdated = true;
                    }
                }
