...
/**
 * Utility patch for WS mount point autostart service creation.
 * 
 * @author Jonas Reese
 * @version $LastChangedRevision:  $ - $LastChangedDate:  $ by $LastChangedBy:  $
 */
public class NEXUSe2eInfoServiceAutostartPatch implements Patch {
    private Object delegate;
    
    public NEXUSe2eInfoServiceAutostartPatch() {
    }
    
    private NEXUSe2eInfoServicePatchDelegate getDelegate() {
        if (delegate == null) {
            delegate = new NEXUSe2eInfoServicePatchDelegate();
            ((NEXUSe2eInfoServicePatchDelegate) delegate).setAutostart( true );
        }
