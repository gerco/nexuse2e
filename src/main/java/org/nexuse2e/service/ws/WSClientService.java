...
public class WSClientService extends AbstractService implements SenderAware {
    ...
    public enum FrontendWebServiceType {
        ...
        XML_DOCUMENT( "Generic XML document (with routing information)" ),
        CIDX_DOCUMENT( "CIDX business document (no routing information)" );
    ...
    public void fillParameterMap( Map<String, ParameterDescriptor> parameterMap ) {
        ...
        parameterMap.put( SERVICE_TYPE_PARAM_NAME,
                new ParameterDescriptor(
                        ParameterType.LIST,
                        "Web service type",
                        "The type of web service to connect to",
                        serviceTypeDrowdown ) );
        parameterMap.put( URL_PARAM_NAME,
                new ParameterDescriptor(
                        ParameterType.STRING, "Web service URL", "The fully qualified web service URL (e.g. http://foo.org/bar)", "" ) );
    }
    ...
    public void sendMessage( MessageContext message ) throws NexusException {
        ...
        if (xmlDocumentService != null) {
            for (MessagePayloadPojo payload : message.getMessagePojo().getMessagePayloads()) {
                xmlDocumentService.processXmlDocument(
                       message.getChoreography().getName(),
                       message.getActionSpecificKey().getActionId(),
                       message.getParticipant().getPartner().getPartnerId(),
                       message.getConversation().getConversationId(),
                       message.getMessagePojo().getMessageId(),
