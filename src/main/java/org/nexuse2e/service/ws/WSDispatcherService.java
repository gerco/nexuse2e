...
public class WSDispatcherService extends AbstractService implements ReceiverAware {
    ...
    public enum FrontendWebServiceType {
        ...
        XML_DOCUMENT( "Generic XML document (with routing information)", XmlDocumentServiceImpl.class ),
        CIDX_DOCUMENT( "CIDX business document (no routing information)", XmlDocumentServiceImpl.class );
    ...
    public void fillParameterMap( Map<String, ParameterDescriptor> parameterMap ) {
        ...
        parameterMap.put( SERVICE_TYPE_PARAM_NAME,
                new ParameterDescriptor(
                        ParameterType.LIST,
                        "Web service type",
                        "The type of web service that shall be installed",
                        serviceTypeDrowdown ) );
    ...
    public void start() {
        ...
        if (wsType != null) {
            try {
                ...
                endpoint = Endpoint.publish( (String) getParameter( URL_PARAM_NAME ), implementor );
                super.start();
            } catch (Exception ex) {
                ex.printStackTrace();
                LOG.error( ex );
