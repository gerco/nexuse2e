...
public class HeaderSerializer extends AbstractPipelet {
    ...
    public MessageContext processMessage( MessageContext messagePipeletParameter )
        ...
        try {
            ...
            String toIDType = messagePojo.getCustomParameters().get( Constants.PROTOCOLSPECIFIC_TOIDTYPE );
            if ( toIDType == null ) {
                //              TODO: for testing..
            ...
            if ( ack ) { // ack
                ...
                // QUALITY OF SERVICE---------------------------------------------------
                String isReliable = messagePojo.getCustomParameters().get( "isReliable" );
                boolean qos = false;
                if ( isReliable != null && isReliable.equalsIgnoreCase( "true" ) ) {
                    qos = true;
                }
                if ( qos ) {
