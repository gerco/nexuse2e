...
public class StaticRoutingPipelet extends AbstractPipelet {
    ...
    public StaticRoutingPipelet() {
        ...
        parameterMap.put( CHOREOGRAPHY_PARAM_NAME, new ParameterDescriptor(
                ParameterType.STRING, "Choreography", "The choreography to set on the processed message", "" ) );
        parameterMap.put( ACTION_PARAM_NAME, new ParameterDescriptor(
                ParameterType.STRING, "Action", "The action to set on the processed message", "" ) );
    }
    ...
    throws IllegalArgumentException, IllegalStateException, NexusException {
        ...
        Engine.getInstance().getTransactionService().initializeMessage( messageContext.getMessagePojo(), messageIdGenerator.getId(),
                conversationIdGenerator.getId(), (String) getParameter( ACTION_PARAM_NAME ), messageContext.getPartner().getPartnerId(), (String) getParameter( CHOREOGRAPHY_PARAM_NAME ) );        
